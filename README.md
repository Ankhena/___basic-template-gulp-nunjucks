...
Все HTML-файлы с разметкой складывайте в папку `source/pages/` с расширеним `njk`. Это нужно для корректной подсветки. При расширении html, может не работать подсветка строк шаблонизатора.
...
Из папки `source/pages/` сборка переносит файлы в папку `build/` (без подпапки pages).
...
### Работа с шаблонами и файлами data

Все глобальные данные лежат в папке `data/global`. Gulp объединяет их в один файл с помощью функции `getGlobalData`, что позволяет потом обращаться с ними как с едиными глобальными данными (использовать во всех шаблонах всех страниц) при этом сохраняя модульность.
...
<ul class="social">
  {% for item in social %}
    <li class="social__item">
      <a class="social__link social__link--{{ item.prefix }}" href="{{ item.link }}" title="{{ item.title }}">
